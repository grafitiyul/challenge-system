...
export default function SetupForm() {
  ...
  // Guard: if setup is not needed, redirect to login
  useEffect(() => {
    apiFetch<{ needsSetup: boolean }>(`${BASE_URL}/auth/setup-needed`)
      .then((res) => {
        if (!res.needsSetup) router.replace('/login');
  ...
  async function handleSetup(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      await apiFetch(`${BASE_URL}/auth/setup`, {
        method: 'POST',
        body: JSON.stringify({ fullName, email, password }),
      });
      router.replace('/login');
    } catch (err: unknown) {
      setError(err instanceof Object && 'message' in err ? String((err as { message: string }).message) : 'שגיאה ביצירת המשתמש');
    } finally {
      setLoading(false);
    }
  }
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={card}>
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          ...
          <h1 style={{ fontSize: 20, fontWeight: 800, color: '#0f172a', margin: 0 }}>הגדרת מנהל ראשון</h1>
          <p style={{ fontSize: 13, color: '#94a3b8', margin: '6px 0 0' }}>צור חשבון מנהל ראשון כדי להתחיל</p>
        </div>
        ...
        <div style={{ background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 8, padding: '10px 14px', fontSize: 13, color: '#92400e', marginBottom: 20 }}>
          המערכת עדיין אינה מוגדרת. צור חשבון מנהל ראשון להמשך.
        </div>
        ...
        <form onSubmit={handleSetup} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div>
            <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#374151', marginBottom: 6 }}>סיסמה (לפחות 8 תווים)</label>
            <input
              dir="ltr"
              type="password"
              value={password}
