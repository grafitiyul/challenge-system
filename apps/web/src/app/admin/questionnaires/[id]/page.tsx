      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 4 }}>
            תוכנית היא המוצר — מילוי שאלון עם מטרה “רשימת המתנה” יוסיף את המשתתפת לרשימת המתנה של התוכנית הזו.
          </div>
        </div>
      </div>
...
export default function TemplateEditorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  function switchTab(tab: TabKey) {
    setActiveTab(tab);
    router.replace(`/admin/questionnaires/${id}?tab=${tab}`);
  }
  ...
  if (notFound || !template) return (
    <div className="page-wrapper" style={{ textAlign: 'center', paddingTop: 60 }}>
      ...
      <Link href="/admin/questionnaires" style={{ color: '#2563eb', fontSize: 14 }}>← חזרה לרשימה</Link>
    </div>
  );
  ...
  return (
    <div className="page-wrapper" style={{ maxWidth: 900, margin: '0 auto' }}>
      {/* Back + header */}
      <Link href="/admin/questionnaires" style={{ color: '#64748b', fontSize: 14, display: 'inline-flex', alignItems: 'center', gap: 4, marginBottom: 16 }}>
        → חזרה לשאלונים
      </Link>
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
          <Link
            href={`/admin/questionnaires/${id}/fill`}
